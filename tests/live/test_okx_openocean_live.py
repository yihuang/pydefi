...
(``https://open-api-pro.openocean.finance/v4``).

Both APIs need a key for quotes: set ``OKX_API_KEY`` and ``OPENOCEAN_API_KEY``.
...
_OPENOCEAN_API_KEY: str | None = os.environ.get("OPENOCEAN_API_KEY")
...
class TestOpenOceanLive:
    """Live tests against the OpenOcean v4 API."""

    async def test_get_quote_weth_usdc(self):
        """GET /quote should return a non-zero output amount for 1 WETH → USDC."""
        client = OpenOcean(chain_id=1, api_key=_OPENOCEAN_API_KEY)
        amount_in = TokenAmount.from_human(WETH, "1")
        quote = await client.get_quote(amount_in, USDC, slippage_bps=50)
