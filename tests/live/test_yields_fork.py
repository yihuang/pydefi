...
from tests.addrs import AAVE_DATA_PROVIDER, AAVE_POOL, ETH_WHALE, USDC
...
def _comet(fork_w3) -> CompoundV3:
    return CompoundV3(w3=fork_w3, chain_id=ChainId.ETHEREUM, comet_address=COMET_USDC)
...
async def _seed_aave_position(fork_w3, amount: int) -> None:
    ...
    approve_tx = {
        "to": USDC.address,
        "data": "0x" + ERC20.fns.approve(bytes(AAVE_POOL), amount).data.hex(),
        "value": "0",
        "gas": "100000",
    }
...
async def _a_usdc_balance(fork_w3) -> int:
    ...
    a_usdc = (await _aave(fork_w3).get_reserve_data(USDC)).a_token_address
