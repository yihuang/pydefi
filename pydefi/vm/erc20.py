...
def emit_approve(prog: Program, token: Address, spender: Address, amount: ValueLike, *, reset: bool = False) -> Operand:
    ...
    if reset:
        prog.assert_(prog.call_contract(bytes(token), ERC20.fns.approve, bytes(spender), 0), "approve reset failed")
    ok = prog.call_contract(bytes(token), ERC20.fns.approve, bytes(spender), amount)
