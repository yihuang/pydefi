...
class LayerZeroOFT(BaseBridge):
    ...
    async def get_status(self, src_tx_hash: str) -> BridgeStatus:
        """Fetch the status of a LayerZero OFT ``send`` transaction.

        Queries the LayerZero Scan API to determine whether the cross-chain
        OFT transfer has been delivered on the destination chain.

        Args:
            src_tx_hash: Transaction hash of the OFT ``send`` call on the
                source chain.
