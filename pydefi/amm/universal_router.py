    ...
    def build_multihop_exact_in_transaction(
        ...
        * Consecutive :class:`V4Hop` objects → single ``V4_SWAP`` command
        ...
        for seg_idx, segment in enumerate(segments):
            ...
            # Only the first segment pulls tokens from the user via Permit2.
            payer_is_user = is_first

            first_hop = segment[0]

            if isinstance(first_hop, V2Hop):
                # Merge into one V2_SWAP_EXACT_IN command with a token path.
                ...
                # Merge into one V3_SWAP_EXACT_IN command with an encoded path.
                ...
                # Merge into one V4_SWAP command using SWAP_EXACT_IN.
                ...
                if payer_is_user:
                    ...
                    settle_action = V4Action.SETTLE
                    # amount=0 → OPEN_DELTA: settle the full outstanding debt
                    settle_params = self.encode_v4_settle_params(
